...
<head>
    ...
    <style>
        ...
        .content p { line-height: 1.6; margin: 0 0 5px 0; }
        ...
        .signatures-left { width: 45%; float: left; text-align: center; }
        .signatures-right { width: 45%; float: right; text-align: center; }
        .signature-line { border-top: 1px solid #000; width: 150px; margin: 60px auto 0 auto; }
        .clear { clear: both; }
        /* Emulate non-breaking space for indentation */
        .indent-2 { margin-left: 2em; }
        .indent-4 { margin-left: 4em; }
        .inline-block { display: inline-block; width: 70px; } /* Adjust width as needed for alignment */
    </style>
</head>
<body>
    <div class="container">
        ...
        <p>Dengan ini mengajukan permohonan izin tidak masuk kerja selama {{ $leaveRequest->start_date->diffInDays($leaveRequest->end_date) + 1 }} hari, pada tanggal {{ \Carbon\Carbon::parse($leaveRequest->start_date)->format('d F Y') }} sampai dengan tanggal {{ \Carbon\Carbon::parse($leaveRequest->end_date)->format('d F Y') }} karena {{ Str::after($leaveRequest->leave_type, 'Izin ') }}.</p>

        <p style="margin-top: 15px;">Saya berkomitmen untuk kembali bekerja setelah izin saya habis serta memastikan seluruh tugas dan tanggung jawab tetap berjalan dengan baik.</p>

        <p style="margin-top: 15px;">Demikian surat izin ini saya sampaikan. Atas perhatian dan kebijaksanaannya, saya ucapkan terima kasih.</p>
