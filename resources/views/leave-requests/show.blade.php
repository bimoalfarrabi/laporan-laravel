                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        @if ($leaveRequest->leave_type === 'Izin terlambat')
                            ...
                            <p><strong>Jam:</strong>
                                {{ \Carbon\Carbon::parse($leaveRequest->start_time)->format('H:i') }} -
                                {{ \Carbon\Carbon::parse($leaveRequest->end_time)->format('H:i') }}</p>
                        @else
                            <p><strong>Tanggal:</strong> {{ $leaveRequest->start_date->format('d M Y') }} -
                                {{ $leaveRequest->end_date->format('d M Y') }}
                                ({{ $leaveRequest->start_date->diffInDays($leaveRequest->end_date) + 1 }} hari)</p>
                        @endif
                        ...
                        <p><strong>Status:</strong>
                            @php
                                $statusClass = '';
                                if ($leaveRequest->status == 'menunggu persetujui') {
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <h3 class="font-semibold text-lg text-gray-800 dark:text-gray-200 mb-4">Riwayat Pengajuan</h3>
                        <div class="text-base text-gray-700 dark:text-gray-300 space-y-3">
                            <p><strong>Dibuat oleh:</strong> {{ $leaveRequest->user->name }} pada <span
                                    class="font-medium">{{ $leaveRequest->created_at->format('d-m-Y H:i') }}</span></p>
                            @if ($leaveRequest->status === 'disetujui')
                                <p><strong>Disetujui oleh:</strong> {{ $leaveRequest->approvedBy->name }} pada <span
                                        class="font-medium">{{ $leaveRequest->approved_at->format('d-m-Y H:i') }}</span>
                                </p>
                            @elseif ($leaveRequest->status === 'ditolak')
                                <p><strong>Ditolak oleh:</strong> {{ $leaveRequest->rejectedBy->name }} pada <span
                                        class="font-medium">{{ $leaveRequest->rejected_at->format('d-m-Y H:i') }}</span>
                                </p>
                            @endif
                        </div>
                    </div>
